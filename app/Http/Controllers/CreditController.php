...
class CreditController extends AppBaseController
{
    ...
    //                 $tasa = 0.30;
    //             } elseif ($periodicity == 'TRADICIONAL') {
    //                 $tasa = $request->input('interest_rate') / 100;
    //             } elseif ($periodicity == 'SEMANAL') {
    //                 $tasa = $request->input('interest_rate')/ 100;
    //             } elseif ($periodicity == 'DIARIO25') {
    //                 $tasa = $request->input('interest_rate') / 100;
    //             } elseif ($periodicity == 'DIARIO4') {
    //                 $tasa = $request->input('interest_rate') / 100;
    //             }
...
public function importCredits(Request $request)
{
    \Excel::load($request->excel, function($reader) {
        ...
        $reader->each(function($row) {
            $credit = new credit;
            $credit->adviser = $row->adviser;
            $credit->date = $row->date;
            $credit->folio = $row->folio;
            $credit->branch = $row->branch;
            $credit->ammount = $row->ammount;
            $credit->interest_rate = $row->interest_rate;
            $credit->dues = $row->dues;
            $credit->periodicity = $row->periodicity;
            $credit->firts_name = $row->firts_name;
            $credit->last_name = $row->last_name;
            $credit->mothers_last_name = $row->mothers_last_name;
            $credit->firm = $row->firm;
            $credit->status = $row->status;
            $credit->client_id = $row->client_id;
            $credit->user_id = $row->user_id;
            $credit->branch_id = $row->branch_id;
            $credit->region_id = $row->region_id;
            $credit->save();


            $debt = new Debt;
            $debt->ammount = $row->ammount_debt;
            $debt->status = $row->status_debt;
            $debt->client_id = $credit->client_id;
            $debt->credit_id = $credit->id;
            ...
            $payment->ammount = $row->ammount_payment;
            $payment->capital = $row->capital;
            $payment->interest= $row->interest;
            $payment->moratorium = '0';
            ...
            // $expendituresCredit = new ExpenditureCredit
            // $expendituresCredit->ammount = $row->ammount_expenditure;
            // $expendituresCredit->concept = 'Colocación';
            // $expendituresCredit->date = $credit->date;
            // $expendituresCredit->credit_id = $credit->id;
            // $expendituresCredit->
    ...
    Toastr::success('Archivo Excel procesado correctamente.', 'CRÉDITOS', ["positionClass" => "toast-bottom-right", "progressBar" => "true"]);
...
if ($vault->ammount == 0 and $product->name == "CREDISEMANA") {
    Toastr::error('No puedes registrar un crédito, ya que no cuentas con efectivo.', 'CRÉDITO', ["positionClass" => "toast-bottom-right", "progressBar" => "true"]);
    return redirect()->back();
} else {
    /* Get signature */
